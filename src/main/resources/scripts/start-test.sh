...
nohup java ${JAVA_MEM_OPTS}  ${JAVA_DUMP_OPTS}  ${JAVA_OPTS} -jar ../datasync.jar --syn.config.file=../config/synConfig.json --spring.profiles.active=test --spring.config.additional-location=../config/application-test.yml 2>&1 > stdout.log &
